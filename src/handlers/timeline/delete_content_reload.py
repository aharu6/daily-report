    ...
    def delete_content_for_reload(
        ...
        while right_key == "will_accept":
            columns[right_col_num].content = ft.DragTarget(
                ...
                on_will_accept=lambda e: Add_will_accept.drag_will_accept(
                    e=e, page=page, columns=columns, drag_data=drag_data
                ),
                on_leave=lambda e: DragLeave.drag_leave(e, page),
                data={"time": model.times()[col_num], "num": col_num, "task": ""},
            )
            ...
            columns[right_col_num].content.on_will_accept = (
                lambda e: Add_will_accept.drag_will_accept(e, page, columns, drag_data)
            )
            columns[right_col_num].on_will_accept = (
                lambda e: Add_will_accept.drag_will_accept(e, page, columns, drag_data)
            )
            right_col_num += 1
            right_key = columns[right_col_num].content.data["task"]
